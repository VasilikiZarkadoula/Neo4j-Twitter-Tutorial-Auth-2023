    ...
    if all(tweet.values()) and not all(pd.isna(v) and  pd.isnull(v) and v != "nan" for v in tweet.values()):  # only add non-empty and non-nan tweets
        for j in range(0,25):
            ...
            if(not pd.isna(tag) and not pd.isnull(tag) and tag != "nan"):
                tag = tag.lower()
                tagRel = graph.run("MATCH (t: Tweet),(h: Hashtag) WHERE h.tag = $tag1 and t.id = $tweetId MERGE (t)-[r:HAS_HASHTAG]->(h) return count(r)",tag1 = tag, tweetId = tweet["id"]).data()

        for j in range(0,3):
            url = str(row.get(f"includes.tweets.0.entities.urls.{j}.expanded_url"))
            if(not pd.isna(url) and not pd.isnull(url) and url != "nan"):
                urlRel = graph.run("MATCH (t: Tweet),(l: Link) WHERE l.url = $url1 and t.id = $tweetId MERGE (t)-[r:HAS_URL]->(l) return count(r)",url1 = url, tweetId = tweet["id"]).data()
